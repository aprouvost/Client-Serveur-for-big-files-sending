    ...
    for data in buff:
        if sock_data.sendto(data, addr):
            ...
            if Common.ACK in dat:
                ...
                print("Warning, received" + sequence_recue)
...
def main(argv):
    ...
    while True:
        ...
        num_sequence = "0".zfill(6)
        seq = int(num_sequence)

        # où on est en reception
        num_reception = "0".zfill(6)
        rec = int(num_reception)
        ...
        for data in buf:
            window_transmitted = False

            # si on est encore dans le window size: on envoie
            while not window_transmitted:
                if cont < window_size:
                ...
                # éventuellement
                    ...
                    while (current - checked) < 0.000005:
                        ...
                        # on update avec le dernier numéro de séquence reçu
                        nbr_ACK +=1
                        if sequence_recue > rec:
                            rec = sequence_recue

                        if rec == seq or sequence_recue == seq:
                            cont = 0
